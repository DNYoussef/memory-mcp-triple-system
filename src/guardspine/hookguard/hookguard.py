...
class HookGuard:
    ...
    def _calculate_risk_score(self, violations: List[HookViolation]) -> float:
        """
    ...
    def validate_hook_content(
        self, content: str, shell_type: str = "bash"
    ) -> Dict[str, Any]:
        """
        ...
        for result in results:
            if result.violations:
                ...
                for v in result.violations[:3]:
                    print(
                        f"     - [{v.risk_level.name}] {v.category}: {v.matched_pattern}"
                    )
