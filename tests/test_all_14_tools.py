...
async def test_tool_list():
    ...
    if len(tools) == 14 and not missing:
        ...
        print_result(
            "tool_list", False, f"Missing tools: {missing}", {"found": len(tools)}
        )
...
async def test_mode_detection(nexus_tool):
    ...
    try:
        ...
        print_result(
            "detect_mode", True, "Alias of mode_detection - uses same ModeDetector"
        )
        return True
...
async def test_entity_extraction(nexus_tool):
    ...
    try:
        ...
        if entities:
            entity_types = set(
                e.get("type", e.get("label", "UNKNOWN")) for e in entities
            )
            print_result(
                "entity_extraction",
                True,
...
async def test_memory_store(nexus_tool):
    ...
    try:
        ...
        nexus_tool.vector_search_tool.indexer.index_chunks(
            chunks, [embeddings.tolist()]
        )
...
async def test_unified_search(nexus_tool):
    ...
    try:
        ...
        graph_engine = (
            GraphQueryEngine(nexus_tool.graph_service)
            if nexus_tool.graph_service
            else None
        )
...
async def test_hipporag_retrieve(nexus_tool):
    ...
    try:
        ...
        results_local = nexus_tool.hipporag_service.retrieve(
            "Claude AI assistant", top_k=5
        )
        print_result(
            "hipporag_retrieve",
            True,
...
async def test_obsidian_sync(nexus_tool):
    ...
    try:
        if nexus_tool._vault_path and Path(nexus_tool._vault_path).exists():
            ...
            print_result(
                "obsidian_sync", True, "Obsidian sync available (no vault configured)"
            )
        return True
    except Exception as e:
...
async def test_beads_query_tasks(nexus_tool):
    ...
    try:
        ...
        print_result(
            "beads_query_tasks", True, f"Beads query tasks works: {len(tasks)} tasks"
        )
        return True
    except FileNotFoundError:
        print_result(
