...
def generate_test_chunk(
    chunk_type: str = "medium", verified: bool = False, chunk_index: int = 0
) -> Dict[str, Any]:
    """
...
def generate_test_batch(batch_size: int = 20) -> List[Dict[str, Any]]:
    ...
    for i in range(batch_size):
        ...
        chunk = generate_test_chunk(
            chunk_type=chunk_type, verified=False, chunk_index=i
        )
        chunks.append(chunk)
...
def populated_service(curation_service):
    ...
    curation_service.collection.add(ids=ids, documents=documents, metadatas=metadatas)
...
def test_batch_load_performance(benchmark, populated_service):
    ...
    assert mean_time < 2.0, f"Mean load time {mean_time:.3f}s exceeds 2s target"
    assert p95_time < 2.5, f"95th percentile {p95_time:.3f}s exceeds 2.5s target"
...
def test_auto_suggest_performance(benchmark, curation_service):
    ...
    for suggestion in result:
        assert suggestion in valid_lifecycles, f"Invalid suggestion: {suggestion}"

    # Performance assertions
    stats = benchmark.stats
    mean_time = stats["mean"]

    assert mean_time < 1.0, f"Mean auto-suggest time {mean_time:.3f}s exceeds 1s target"
...
def test_full_workflow_performance(benchmark, curation_service):
    ...
    def full_workflow():
        ...
        curation_service.log_time(duration_seconds=duration, chunks_curated=20)
...
def test_edge_case_performance(benchmark, curation_service):
    ...
    assert mean_time < 0.1, f"Edge case processing {mean_time:.3f}s too slow"
