...
class TestVectorIndexerIntegration:
    ...
    def indexer(self, temp_dir):
        ...
        indexer = VectorIndexer(
            persist_directory=temp_dir, collection_name="test_integration"
        )
    ...
    def test_metadata_filtering_exact_match(
        self, indexer, sample_chunks, sample_embeddings
    ):
    ...
    def test_metadata_filtering_comparison(
        ...
        query_emb = [0.2] * 384
        results = indexer.search_similar(
            query_embedding=query_emb, top_k=5, where={"level": {"$gte": 2}}
        )
    ...
    def test_metadata_filtering_with_search(
        ...
        results_filtered = indexer.search_similar(
            query_embedding=query_emb, top_k=3, where={"language": "Python"}
        )

        # Search without filter for comparison
        results_unfiltered = indexer.search_similar(query_embedding=query_emb, top_k=3)
