...
class TestVectorIndexerRealChromaDB:
    ...
    def test_add_document_persists_to_chromadb(
    ...
    def test_search_returns_similar_documents(
        ...
        assert (
            "NASA" in results[0].get("document", "")
            or results[0].get("distance", 1) < 0.5
        )

    def test_delete_document_removes_from_chromadb(
        self, real_vector_indexer, real_embedding_pipeline
    ):
    ...
    def test_batch_indexing(
        self, real_vector_indexer, real_embedding_pipeline, sample_documents
    ):
...
class TestVectorSearchToolRealChromaDB:
    ...
    def test_execute_returns_real_results(
        self, indexed_documents, real_embedding_pipeline, temp_data_dir
    ):
