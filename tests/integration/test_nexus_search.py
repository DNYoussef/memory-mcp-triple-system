...
class TestCompressStep:
    ...
    def test_compress_returns_core_and_extended(self):
        ...
        processor = NexusProcessor()
        candidates = [
            {"id": str(i), "text": f"Content {i}", "score": 1.0 - i * 0.1}
            for i in range(20)
        ]
    ...
    def test_compress_mode_affects_extended_count(self):
        ...
        processor = NexusProcessor()
        candidates = [
            {"id": str(i), "text": f"Content {i}", "score": 1.0 - i * 0.01}
            for i in range(50)
        ]

        exec_result = processor.compress(
            candidates, mode="execution", token_budget=10000
        )
        brain_result = processor.compress(
            candidates, mode="brainstorming", token_budget=20000
        )
...
class TestFullPipeline:
    ...
    def test_process_returns_expected_structure(self):
        ...
        result = processor.process(
            query="test query", mode="execution", top_k=10, token_budget=5000
        )
        assert "core" in result
        assert "extended" in result
