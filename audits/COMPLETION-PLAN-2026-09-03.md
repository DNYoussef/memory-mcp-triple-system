# Memory MCP Triple System - Completion Plan v7 (2026-09-03; round-5 folded; READY FOR PHASE 0)
...
audits/gemini-mece-round5-2026-09-03.md) -> v7 here. Every R5 finding was verified against the
...
environment measurements. This is that revision. Ledger: audits/GATES-2026-09-03.md (v7).
...
- Worktree (Git Bash; PowerShell 5.1 has no &&): `git worktree add -d ../memory-mcp-completion
  origin/main; cd ../memory-mcp-completion; git checkout -b completion-2026-09`.
...
outside it is reported dead. ROOT SET (the only hand-written part; each entry carries its evidence):
  ...
  R7 conftest.py, tests/conftest.py, tests/unit/conftest.py, tests/fixtures/real_services.py
     ...
     graph. Codex R5 #1.)
Graph semantics (all four were underspecified in v6):
...
P0.1 Worktree + branch; `git status` clean.
...
P0.3 Two exclusive-created artifacts:
     ...
     - audits/phase3-pre-deletion.json, `open(path,"x")` at the START of Phase 3, recording the
       ...
       comparison checkout is not the real pre-deletion tree). The gate asserts `git status
       --porcelain` was empty when the artifact was written.
...
P0.6 Deletion floor: nodeid SET difference between the working tree and a temp
     ...
     is DERIVED from reachability.py's dead set plus the listed mixed-file functions - never read
     from a manifest (that circularity was R4's deepest finding). --self-test uses dummy sets and
     never calls git.
...
P1d Lightweight Bayesian parity: lightweight_query_engine.py:38-65 and :67+ adopt the pgmpy
     signature (probabilistic_query_engine.py:61-66) and result shape (:225-256); delete the
     TypeError adapter tier_queries.py:195-210; add MEMORY_MCP_BAYESIAN_BACKEND in
     src/bayesian/__init__.py:12-39; update tests pinning the old shape.
...
P1b Beads: _run_command (beads_bridge.py:282-310) raises BeadsCLIError on rc!=0 / timeout /
     ...
     True; HTTP unified_router.py:65-82 keeps degrading but adds "beads_error" to the payload at
     :55-63; update tests/unit/test_beads_bridge.py:55-68 to pytest.raises. All 9 combinations.
P1c Request-local degradation across every TRI-TIER path (the wording is now precise - Codex R5 #4:
     ...
     - service_wiring.py:492-529 gains execute_with_status() -> (results, degraded); execute()
       delegates and returns results only.
     ...
     - THREE tri-tier HTTP routes include degraded_tiers and return 503 when all three are down:
       ...
       at :794 and :905. /tools/graph_query (:854-870) is SINGLE-TIER, already converts exceptions
       to 500, and is explicitly out of scope - stated here so it is not silently omitted.
     - Fixture: tests/unit/test_stdio_server.py:102 sets only execute.return_value; add
       `execute_with_status.return_value = ([], [])` and update assertions at :159, :165, :171, :222.
...
P2a One trace site: request_router.handle_call_tool (:1028-1050), which protocol_handler.py:42-57
     ...
     (query_trace.py:172 defaults to memory.db), warn when log() returns False (:229-235). Remove
     the handler-local trace at :173-188 AND replace `trace.retrieval_ms` at :201 with a local
     elapsed_ms. The stdio_server.py:144-163 facade stays; each branch keeps its literal
...
P2b cleanup_expired under stdio, rolling 24h, atomic, RE-ACQUIRABLE. `KVStore.set_if_absent(key,
     ...
     _run_lifecycle_maintenance (memory_ingestion_service.py:265-273) claims
     "lifecycle:cleanup:claim" with ttl=86400; only the winner calls cleanup_expired(). __init__
     (:34-55) gains kv_store=None and clock=None; http_server.py:262 passes get_kv_store(),
...
Procedure: run reachability.py --report; review the computed dead list; delete those files plus
the surgery below; RE-RUN reachability after each batch (G3: deleting a function removes edges);
repeat until dead_remaining is 0 or exactly the named exceptions.
...
loop_interfaces,metadata_sync}.py, src/stores/event_log.py.init (a 130-byte rename artifact), and
...
TESTS - surgery:
- tests/unit/test_phase5_memory_mcp_tail.py: keep ONLY :18-30 (GraphService); delete the imports
  :7-14 and the blocks :33-53, :56-85, :88-119.
...
- Edits: README.md:12-15, :44-52, :285-301; docs/CURRENT.md:18-21, :51, :52 (stdio-only tracing),
  ...
  (nonexistent scripts/test-mode-detection.py); every other sentence naming a module in the ten
...
- P6 railway_smoke.py: Dockerfile.railway:10-15 pins EMBEDDING_MODE=api and EMBEDDING_API_BASE to
  ...
  returning `{"data":[{"index":0,"embedding":[...384 floats...]}]}` seeded by hashlib from the
  input text, accepting `dimensions` and `encoding_format`. Container:
...
- Coverage: after Phase 3 measure `pytest --cov=src` TOTAL once, set pytest.ini:13
  --cov-fail-under to floor(TOTAL) with `# ratchet up only`; the gate asserts configured <= measured.
...
The FIX plan (P1a-d, P2a-b) has been stable since v4 apart from genuinely missed live paths. v7
closes every R5 blocker. Recommendation: START PHASE 0. A seventh audit round would be auditing
prose about gates that do not exist yet; the gates themselves are the better oracle, and Phase 0's
first deliverable (reachability.py --report) replaces the last hand-written artifact.
...
- v7: all folded. Ledger v7. NEXT ACTION IS PHASE 0, not another audit round.
